...
Please guide me on how I can customize seating, fabric materials, and colors!`;
  ...
  return (
    <div className="bg-[#FFFDFC] rounded-xl border border-[#E0D8CE] overflow-hidden flex flex-col justify-between shadow-md hover:shadow-xl hover:border-[#8B6844] hover:-translate-y-2 transition-all duration-300 group h-[430px] max-h-[445px]">
      ...
      <div className="p-4 flex-grow flex flex-col justify-between text-left gap-1.5">
        <div className="space-y-1">
          {/* Product Name */}
          <h3 className="text-[15px] font-semibold tracking-wide font-serif text-[#201712] leading-tight line-clamp-1">
            {product.name}
          </h3>
          
          {/* Price */}
          <p className="text-sm font-bold text-[#8B6844] font-serif leading-none">
            ₹{currentPrice.toLocaleString('en-IN')}
          </p>
          
          {/* Warranty with shield icon */}
          <div className="flex items-center space-x-2 text-[10px] text-[#6D6258]">
            <svg className="h-3.5 w-3.5 text-[#8B6844]" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
          ...
          {/* Features list */}
          <ul className="space-y-0.5 pt-1.5 border-t border-[#E0D8CE] text-[11px] text-[#6D6258] font-light leading-[1.3]">
        ...
        <div className="grid grid-cols-2 gap-2 pt-2 border-t border-[#E0D8CE] lg:hidden">
          <Link
            to={detailPath}
            className="py-2 text-[11px] font-bold uppercase tracking-wider text-white btn-luxury-wood rounded transition-all text-center shadow-sm"
          >
            Details
          </Link>
          <a
            ...
            target="_blank"
            rel="noopener noreferrer"
            className="py-2 text-[11px] font-bold uppercase tracking-wider text-[#8B6844] hover:bg-[#F4F1EC] border border-[#8B6844] rounded transition-all text-center flex items-center justify-center space-x-1"
          >
