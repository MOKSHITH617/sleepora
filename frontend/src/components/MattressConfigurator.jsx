    ...
    <div className="bg-white rounded-md border border-border shadow-md overflow-hidden grid grid-cols-1 lg:grid-cols-12 gap-0 animate-fade-in">
      ...
      <div className="lg:col-span-7 p-6 md:p-8 flex flex-col justify-between">
        <div>
          ...
          <p className="text-xs text-text-muted mb-6">Choose core materials, sizes, and slide the mattress depth</p>

          {/* 1. Core Selection */}
          <div className="mb-6">
            <label className="block text-[11px] font-bold text-primary uppercase tracking-wider mb-2.5">1. Choose Mattress Core</label>
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-3">
              {config.cores.map((c, idx) => (
                <label 
                  key={idx}
                  className={`border rounded-sm p-4 flex flex-col justify-between cursor-pointer transition-all duration-200 select-none ${selectedCore === c.type ? 'border-accent bg-accent-light/30 shadow-sm' : 'border-border bg-bg-light hover:border-primary-lighter'}`}
                >
                  <input 
                    type="radio" 
                    name="config-core" 
                  ...
                  />
                  <div>
                    <span className="block font-bold font-display text-sm text-primary leading-tight mb-1">{c.name}</span>
                    <span className="block text-[10px] text-text-muted leading-relaxed line-clamp-2">{c.desc}</span>
                  </div>
                </label>
              ))}
            </div>
          ...
          <div className="mb-6">
            <label htmlFor="config-size" className="block text-[11px] font-bold text-primary uppercase tracking-wider mb-2">2. Select Dimensions (Inches)</label>
            <select
              id="config-size"
              value={selectedSize}
          ...
          <div className="mb-6">
            <div className="flex justify-between items-center mb-2">
              <label htmlFor="config-thickness" className="text-[11px] font-bold text-primary uppercase tracking-wider">3. Thickness Depth</label>
              <span className="bg-primary text-accent px-3 py-1 font-bold rounded-sm text-sm font-display tracking-wide">{thicknessVal} inches</span>
            </div>
      ...
      <div className="lg:col-span-5 bg-primary p-6 md:p-8 flex flex-col justify-between relative overflow-hidden border-t lg:border-t-0 lg:border-l border-border min-h-[360px]">
        ...
        <div>
          ...
          <h4 className="text-xl font-bold font-display text-white mb-1">{coreData?.name || 'Loading Core...'}</h4>
          ...
          <div className="my-8 flex flex-col gap-1.5 items-center justify-center min-h-[160px]">
            {coreData?.layers.map((layer, idx) => {
              ...
              return (
                <div
                  key={idx}
                  className={`layer-3d max-w-[280px] ${idx === 0 ? 'layer-quilted' : ''}`}
                  style={{
